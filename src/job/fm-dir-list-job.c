...
//g_warning("FIX %s, path_str", path_str);
if (job->updir == 1) {
char* path_strz;
FmPath* new_pathz;

path_strz = g_path_get_dirname(path_str);
//g_warning("FIX %s, path_str", path_strz);
new_pathz = fm_path_new_for_str(path_strz);
fi = _new_info_for_native_file(job, new_pathz, "..", &err);
fm_file_info_set_disp_name(fi, "..");
fm_dir_list_job_add_found_file(job, fi);
fm_file_info_unref(fi);
fm_path_unref(new_pathz);
g_free(path_strz);
}
        }
