...
//----------------------
/*FmFileInfo* fin;
fin = fm_file_info_new();
fm_file_info_set_path(fin, fm_path_new_for_str("/root"));
fm_file_info_set_disp_name(fin, "root");
fm_file_info_list_push_tail(folder->files, fin);
fm_file_info_unref(fin);
files_to_add = g_slist_prepend(files_to_add, fin);
            g_signal_emit(folder, signals[FILES_ADDED], 0, files_to_add);
            g_slist_free(files_to_add); */
//----------------------------
...
static FmFolder* fm_folder_new_internal(FmPath* path, GFile* gf, guint u)
{
...
static FmFolder* fm_folder_get_internal(FmPath* path, GFile* gf, guint u)
{
    FmFolder* folder;
    ...
    if( G_UNLIKELY(!folder) )
    {
        ...
        folder = fm_folder_new_internal(path, gf, u);
        if(_gf)
            g_object_unref(_gf);
        G_LOCK(hash);
...
FmFolder* fm_folder_from_gfile(GFile* gf)
{
    ...
    FmFolder* folder = fm_folder_get_internal(path, gf, 0);
    fm_path_unref(path);
    return folder;
}
...
FmFolder* fm_folder_from_path_name(const char* path)
{
    ...
    FmFolder* folder = fm_folder_get_internal(fm_path, gf, 0);
    g_object_unref(gf);
...
void fm_folder_set_updir(FmFolder* folder, guint u)
{
	//g_warning("fm_folder_set_updir %i", u);
	folder->updir = u;
}
...
//g_warning("fm_folder_reload %i", folder->updir);
    ...
    folder->dirlist_job->updir = folder->updir;
...
//    return fm_folder_get_internal(path, NULL);
    ...
    FmFolder* folder = fm_folder_get_internal(path, NULL, 0);
    return folder;
}

FmFolder* fm_folder_from_path2(FmPath* path)
{
//    return fm_folder_get_internal(path, NULL);

    FmFolder* folder = fm_folder_get_internal(path, NULL, 1);
    return folder;
}
