...
FmPath* fm_path_new_for_display_name(const char* path_name)
{
    ...
    char sep_char;
//g_warning("fm_path_new_for_display_name %s", path_name);
...
//g_warning("_fm_path_display_name %s", disp);
    return disp;
}

char* fm_path_display_name(FmPath* path, gboolean human_readable)
{
//	g_warning("fm_path_display_name");
...
if (path->disp_name) {
	if (path->disp_name != BASENAME_AS_DISP_NAME) {
        if (g_strcmp0("..", path->disp_name) == 0)
        {
g_warning(" %s", path->disp_name);
p = px;
        }
}
} */
	//g_warning("_fm_path_set_display_name %s", p);
    if (!p || !*p)
    {
        char *_name = NULL;
        if(!fm_path_is_native(path))
    ...
    if (path->disp_name != BASENAME_AS_DISP_NAME)
    {
        ...
        if (g_strcmp0(p, path->disp_name) == 0)
        {
            G_UNLOCK(roots);
            return;
        }
    ...
    if (g_strcmp0(p, path->name) == 0)
        path->disp_name = BASENAME_AS_DISP_NAME;
    else
        path->disp_name = g_strdup(p);
    G_UNLOCK(roots);
}
